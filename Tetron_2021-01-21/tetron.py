...
# Helper Functions.
# =============================================================================
# Return a 3-tuple with RGB values. For grayscale colors, pass a number between 0 (black) and 1 (white). For a specific RGB color, pass its corresponding string code.
def rgb(color, tint=0):
    ...
    if tint != 0:
        if tint > 0:
            color = np.array([255,255,255])*abs(tint) + color*(1-abs(tint))
        elif tint < 0:
            color = np.array([0,0,0])*abs(tint) + color*(1-abs(tint))
    return tuple(color)
...
class Tetron:
    ...
    def __init__(self):
        ...
        self.weight_increment_advanced = 0.015
        ...
        # pygame.mixer.music.load(os.path.join(folder_sounds, 'tetron.wav'))
    ...
    def start_game(self):
        ...
        # Reset the cleared lines counter.
        self.cleared_count = 0
        # Reset the timers.
        pygame.time.set_timer(pygame.USEREVENT+1, self.speed_fall)
        pygame.time.set_timer(pygame.USEREVENT+2, self.time_increase_difficulty)
        # Reset the arrays.
        self.array_current[:] = 0
        self.array_dropped[:] = 0
    ...
    def stop_game(self):
        ...
        print('Lines cleared:  ', self.cleared_count)

    # Convert a tetrimino ID from its number to its string, or vice versa.
    # def convert_tetrimino_id(self, number_or_str):
    #     id_pairs = [
    #         (10, 'I'), (20, 'J'), (30, 'L'), (40, 'O'), (50, 'S'), (60, 'T'), (70, 'Z'),
    #         (100, 'freebie'), (101, 'I+'), (201, 'J+'), (301, 'L+'), (401, 'O+'), (402, 'O+'), (501, 'S+'), (601, 'T+'), (701, 'Z+'), (801, '.'), (802, ':'), (803, '*'), (901, 'ghost'), (902, 'G')
    #         # (1, 'I'), (2, 'J'), (3, 'L'), (4, 'O'), (5, 'S'), (6, 'T'), (7, 'Z'),
    #         # (10, 'I+'), (20, 'J+'), (30, 'L+'), (40, 'O+'), (41, 'O+'), (50, 'S+'), (60, 'T+'), (70, 'Z+'), (100, 'freebie'), (101, '.'), (102, ':'), (103, '*'), (104, 'ghost'), (105, 'G')
    #         ]
    ...
    def create_new(self):
        ...
        id_advanced = [101, 102, 199, 201, 202, 301, 302, 401, 402, 403, 501, 601, 602, 701, 801, 811, 812, 813, 814]
    ...
    def update(self):
        ...
        # Add the current tetrimino to the displayed array.
...
logo = pygame.transform.scale(logo, [height_panel*2, height_panel])
...
while not done:
    ...
    for row in range(game.row_count):
        for column in range(game.column_count):
            ...
            if number != 0:
                if game.flag_blind:
                    color = 0.3  # Color of placed blocks in blind mode
                else:
                    if number < 0:
                        color = 0.4  # Color of highlighted blocks
                    else:
                        color = number + 0  # Color of placed blocks
            else:
                color = 0.25  # Color of blank blocks
            pygame.draw.rect(surface=screen, color=rgb(color, tint=tint), rect=[(game.margin+game.width)*column+game.margin, (game.margin+game.height)*row+game.margin+height_panel, game.width, game.height])
    ...
    text_cleared = font.render('{}'.format(game.cleared_count), True, rgb(1))
